...
def L(v):
    return np.array(v, object).tolist()
...
def buffer_of_size(sz, n=8):
    ...
    for i in range(n):
        ...
        # avoid this :)
        vlen = Fraction(FractionFromFloat(math.sqrt(float(vlen2)), -10)) / sz
...
class Simple_Polygon_With_Holes:
    ...
    def transformed_with(self, f):
        ...
        return self.__class__(
            fpoly(self.outer), [fpoly(hole) for hole in self.holes]
        )
...
class Geom2D_Py_Simple_Holes(geom2dbase.Geom2DBase):
    ...
    def repro(self):
        ...
        return "plycutter.geom2d_py.Geom2D_Py_Simple_Holes([%s])" % ",".join(
            [spwh.repro() for spwh in self.spwhs]
        )
    ...
    def union(cls, geoms):
        ...
        return cls.empty()._arrangement_op(geoms, lambda *args: any(args))
    ...
    def polygon(cls, points, reorient=False):
        ...
        for point in points:
            for coord in point:
                assert (
                    isinstance(coord, FractionClass) or int(coord) == coord
                ), (coord, points)
        ...
        result = Geom2D_Py_Simple_Holes([Simple_Polygon_With_Holes(points)])
    ...
    def from_shapely(cls, sh):
        if sh.geom_type == "GeometryCollection":
            geoms = [cls.from_shapely_polygon(geom) for geom in sh.geoms]
            spwhs = []
            for geom in geoms:
        ...
        if sh.geom_type == "MultiPolygon":
            geoms = [cls.from_shapely_polygon(poly) for poly in sh.geoms]
            spwhs = []
            for geom in geoms:
                assert len(geom.spwhs) == 1
                spwhs.extend(geom.spwhs)
    ...
    def __or__(self, other):
        ...
        return self._arrangement_op([self, other], operator.or_)

    def __and__(self, other):
        if oplogger.isEnabledFor(logging.DEBUG):
            oplogger.debug("and %s %s", self.area(), other.area())
        return self._arrangement_op([self, other], operator.and_)
    ...
    def buffer(self, amount, resolution=8):
        ...
        if oplogger.isEnabledFor(logging.DEBUG):
            oplogger.debug(
                "buffer %s %s", amount, len(list(self.all_segments()))
            )
        if amount == 0:
            return self
        amount = FractionFromExact(amount)
        buf = buffer_of_size(abs(amount), n=resolution)
        res = self._minkowski_convex_op(buf, 1 if amount > 0 else -1)
        oplogger.debug("done")
        return res
    ...
    def _minkowski_convex_op(self, cpoly, sign):
        ...
        return self._arrangement_op(
            subs, lambda *v: np.any(posmask & v) & ~np.any(negmask & v)
        )
...
def minkowski_ribbon_simple_convex(spoly, cpoly, sign):  # noqa # XXX
    ...
    for vp, v, vn in _triple_vertices_iter(spoly):
        ...
        if (nfr - pfr) % MAX_FRANGLE < FRANGLE_180:
            # Turn right
            idxs = np.arange(
                iprev, 1 + (inext if inext >= iprev else inext + len(cpoly))
            )
            if sign < 0:
                idxs = np.array([inext])
        else:
            # Turn left
            idxs = np.arange(
                inext, 1 + (iprev if iprev >= inext else iprev + len(cpoly))
            )
            if sign > 0:
                idxs = np.array([inext])
